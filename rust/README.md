# The Rust workspace of ngx_waf

The workspace holds three crates.  `src/` is `ngx-waf-core`, the business
logic of the `ngx_waf` nginx module; its manifest is `src/Cargo.toml` and the
crate root is the source directory itself.  `rule/` is the preview of the
next generation rule engine and `rule-cli/` is its command line tool, see the
section below.

nginx (C) keeps only what it must keep: module/directive registration, request
data packing, the asynchronous parts of the nginx event loop and the final
response/variable plumbing.  Everything else — configuration semantics, rule
parsing and matching, caching, CC protection, audit strings — lives in the
core crate.
...
| file | contents |
| --- | --- |
| `src/Cargo.toml` | the manifest of the core crate (the crate root is `src/`, the library path is `lib.rs`) |
...
| `src/fuzz/` | the cargo-fuzz crate of the shared memory state machine |
| `rule/` | the next generation DSL rule engine (preview, not wired into nginx yet) |
| `rule-cli/` | the `ngx-waf-rule` tool that checks and tests the rules of `rule/` |
...
## The next generation rule engine

The project discussion
[#129](https://github.com/ADD-SP/ngx_waf/discussions/129) describes a flexible
rule syntax, `Rule "<condition>" <actions>;`.  The `rule/` workspace member is
that engine: it compiles a rule file into a `RuleSet`, evaluates it against a
borrowed request view and keeps the user variables (the scoring example of the
discussion) inside one evaluation.  The `rule-cli/` member builds the
`ngx-waf-rule` binary with the `check` and `test` subcommands, the learning
tool of the same discussion.
...
The nginx `config` script drives cargo and builds the core alone with
`rust/src/Cargo.toml`; `mise run build` at the repository root is the supported
entry point (the tasks live in `.mise/tasks/`, the tools and the system packages
in `mise.toml`).  The Rust toolchain itself is not managed
...
`rand`, `subtle`, `cidr` and `cookie`; the two preview crates additionally
need `pest` (with `pest_derive`), `regex`, `cidr` and `clap`, plus
`assert_cmd`, `predicates` and `tempfile` for the tests.  nginx
...
out.  `mise run fuzz` drives `rust/src/fuzz/fuzz_targets/table.rs` with
cargo-fuzz, `FUZZ_TIME` sets the budget in seconds.  `mise run
test-sanitize-nginx` builds the tree of `build-static` with AddressSanitizer,
runs the end to end suite against it and restores the ordinary build on the
way out; the
