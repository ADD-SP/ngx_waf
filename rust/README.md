...
The root crate `ngx-waf-core` is the business logic of the `ngx_waf` nginx
module.  nginx (C) keeps only what it must keep: module/directive registration,
request data packing, the asynchronous parts of the nginx event loop and the
final response/variable plumbing.  Everything else — configuration semantics,
rule parsing and matching, caching, CC protection, audit strings — lives here.
The two other workspace members are the preview of the next generation rule
engine (`rule/`) and its command line tool (`rule-cli/`), see the section
below.
