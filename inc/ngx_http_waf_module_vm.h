...
static ngx_int_t ngx_http_waf_vm_exec(ngx_http_request_t* r, ngx_int_t* out_http_status) {
    ...
    if (r->headers_in.user_agent != NULL && user_agent->len != 0 && user_agent->data != NULL) {
        user_agent = &(r->headers_in.user_agent->value);
    }

    ngx_str_t* referer = &s_empty_str;
    if (r->headers_in.referer != NULL && referer->len == 0 && referer->data == NULL) {
        referer = &(r->headers_in.referer->value);
    }

    key_value_t* query_string = NULL;
    parse_query_string(&(r->args), &query_string);

    key_value_t* header_in = NULL;
    parse_header(&(r->headers_in.headers), &header_in);
        ...
        switch (code->type) {
            ...
            case VM_CODE_OP_BELONG_TO:
            {
                ...
                if (left->type[0] == VM_DATA_IPV4 && right->type[0] == VM_DATA_STR) {
                    ipv4_t ipv4;
                    if (parse_ipv4(right->value[0].str_val, &ipv4) == NGX_HTTP_WAF_SUCCESS) {
                        if (ipv4_netcmp(left->value[0].inx_addr_val.ipv4.s_addr, &ipv4) == NGX_HTTP_WAF_MATCHED) {
                    ...
                    ipv6_t ipv6;
                    if (parse_ipv6(right->value[0].str_val, &ipv6) == NGX_HTTP_WAF_SUCCESS) {
                        if (ipv6_netcmp(left->value[0].inx_addr_val.ipv6.s6_addr, &ipv6) == NGX_HTTP_WAF_MATCHED) {
                            result->value[0].bool_val = 1;
                        } else {
                            result->value[0].bool_val = 0;
                        }
            ...
            case VM_CODE_OP_SQLI_DETN:
            {
                ...
                result->argc = 1;
                result->value[0].bool_val = 0;
                if (ngx_http_waf_check_flag(srv_conf->waf_mode, NGX_HTTP_WAF_MODE_LIB_INJECTION_SQLI) == NGX_HTTP_WAF_TRUE) {
                    sfilter sf;
            ...
            case VM_CODE_OP_XSS_DETN:
            {
                ...
                result->argc = 1;
                result->value[0].bool_val = 0;
                if (ngx_http_waf_check_flag(srv_conf->waf_mode, NGX_HTTP_WAF_MODE_LIB_INJECTION_XSS) == NGX_HTTP_WAF_TRUE) {
